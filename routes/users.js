...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const hobbies =
        req.body.hobbies.length == 1
          ? req.body.hobbies.split(",")
          : [req.body.hobbies];
      const interests =
        req.body.interests.length == 1
  ...
  async (req, res) => {
    try {
      ...
      const hashedPassword = await bcrypt.hash(
        req.body.newPassword? req.body.newPassword : req.body.password,
        salt
      );
      ...
      const [updatedCount, updatedUser] = await Users.update(
        {
          email: req.body.email,
          password: req.body.newPassword? hashedPassword: req.body.password,
        },
        { where: { id: req.user.id }, returning: true }
      );
